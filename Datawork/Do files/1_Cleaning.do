	...
	*-------------------------------------------------------------------------------------------------------------------------*	
	use  "$dtinter\Fin_Lit_pooled_data.dta", replace 
		...
		*----------------------------------------------------------------------------------------------------------------------*	
		{
			* Exposição efetiva ao programa (Z), independente do braço: turmas de controle também podem ter Z == 1,
			* e é essa contaminação que faz o LATE diferir do ITT. Duas medidas, da mais restritiva para a mais ampla.

			* Evidência do professor: algum professor da turma respondeu quantas aulas deu (T38, 1 a "8 ou mais")
			* ou quanto do livro cobriu (T40). Responder qualquer categoria já indica que houve aula.
			egen 	 classes_taught    = rowmax(prof1_rp_38 prof2_rp_38 prof3_rp_38 prof1_rp_40 prof2_rp_40 prof3_rp_40)
			gen byte teacher_exposure  = !missing(classes_taught) 

			* Turma sem nenhum professor vinculado no questionário
			gen byte no_teacher_linked = missing(teacher1_key) & missing(teacher2_key) & missing(teacher3_key)

			* Evidência do aluno: declara ter tido aulas de educação financeira (SE60 == 2). Só existe do 5º ano
			* em diante, então turmas de 3º ano sem professor vinculado ficam sem essa informação.
			gen byte student_class     = socio_rp_60 == 2 if inlist(socio_rp_60, 1, 2)
			bysort cd_escola cd_turma: egen share_class = mean(student_class)

			* (1) Medida conservadora -> LATE conservador
			*     professor dá evidência de aula; se a turma não tem professor vinculado, mais de metade dos
			*     alunos que responderam declara ter tido aulas
			gen byte treated  = 0
			replace  treated  = 1 if teacher_exposure   == 1
			replace  treated  = 1 if no_teacher_linked  == 1 & share_class > 0.5 & !missing(share_class)

			* (2) Medida ampla -> LATE amplo
			*     a mesma evidência do professor, ou o próprio aluno declarando ter tido aulas
			gen byte treated2 = 0
			replace  treated2 = 1 if teacher_exposure   == 1
			replace  treated2 = 1 if student_class      == 1

			label var treated  "Exposure to the program (conservative)"
			label var treated2 "Exposure to the program (broad)"

			tab treated  d, mis col
			tab treated2 d, mis col
			tab treated  treated2, mis
		}
		...
		*----------------------------------------------------------------------------------------------------------------------*	
		{	
			...
			*Labels
			...
			label var resp_educ_3 		"Mother education/Legal guardian: at least high school degree"
			...
			label var mother_educ_3 	"Mother education/Legal guardian: at least high school degree"
