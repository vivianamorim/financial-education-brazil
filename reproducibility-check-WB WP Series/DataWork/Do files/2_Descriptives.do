		...
		use 	"$dtfinal/Fin_Lit_pooled_data_clean.dta",replace 
			...
			collapse (mean)vl_proficiencia IDEB_F05_2015 IDEB_F09_2015 if serie == 5 | serie == 9, by(serie cd_escola)
				...
				|| (scatter vl_proficiencia IDEB_F05_2015 if serie == 5, symbol(O) color(emidblue*0.8)  msize(medsmall) mlabcolor(black) mlabposition(9) mlabsize(2)	 ///   
					...
					graph export "$figures/FigureC1a.pdf", as(pdf) replace	
				...
				|| (scatter vl_proficiencia IDEB_F09_2015 if serie == 9, symbol(O) color(emidblue*0.8)  msize(medsmall) mlabcolor(black) mlabposition(9) mlabsize(2)	 ///   
					...
					legend(off) ///
					note("", span color(black) fcolor(background) pos(7) size(small)))
					graph export "$figures/FigureC1b.pdf", as(pdf) replace	
	}
	...
	*---------------------------------------------------------------------------------------------------------------------*
	{	//sample selected
		use 	"$dtinter/Treatment and Control Groups.dta", clear
			...
			sort order
			drop order
			export excel "$descriptives/Table B1.xlsx", firstrow(variables) replace
	}
	...
	*---------------------------------------------------------------------------------------------------------------------*
	{
		...
		export excel "$descriptives/Table B3.xlsx", replace	
	}
		...
		*-----------------------------------------------------------------------------------------------------------------*
		{	
			...
			*-----------------------------------------------------------------------------------------------------------------*
				...
				* Run balance tests
				iebaltab fl_blank_score  fl_blank_parent if cd_etapa_aplicacao_turma == 3, format(%12.1f)  cov(estrato) rowvarlabels grpvar(resultado) save("$descriptives/Table3_block1_3grade.xlsx") replace
				iebaltab fl_blank_score  fl_socio_blank  if cd_etapa_aplicacao_turma == 5, format(%12.1f)  cov(estrato) rowvarlabels grpvar(resultado) save("$descriptives/Table3_block1_5grade.xlsx") replace
				iebaltab fl_blank_score  fl_socio_blank  if cd_etapa_aplicacao_turma == 7, format(%12.1f)  cov(estrato) rowvarlabels grpvar(resultado) save("$descriptives/Table3_block1_7grade.xlsx") replace
				iebaltab fl_blank_score  fl_socio_blank  if cd_etapa_aplicacao_turma == 9, format(%12.1f)  cov(estrato) rowvarlabels grpvar(resultado) save("$descriptives/Table3_block1_9grade.xlsx") replace
			...
			*-----------------------------------------------------------------------------------------------------------------*
				foreach grade in 3 5 7 9 {
					import 		excel "$descriptives\Table3_block1_`grade'grade.xlsx", allstring clear
						...
						if 		`grade' == 3		export	 excel using "$descriptives\Table B5.xls",  replace
						if 		`grade' == 5 		export	 excel using "$descriptives\Table B5.xls",  sheetmodify cell(E1)	
						if 		`grade' == 7 		export	 excel using "$descriptives\Table B5.xls",  sheetmodify cell(H1)	
						if 		`grade' == 9 		export	 excel using "$descriptives\Table B5.xls",  sheetmodify cell(K1)	
						erase 	"$descriptives\Table3_block1_`grade'grade.xlsx"
				}
		}
		...
		{	
			...
			*-----------------------------------------------------------------------------------------------------------------*
				...
				*-----------------------------------------------------------------------------------------------------------------*
				...
				using "$descriptives/Table3_block2.xlsx", replace stats(N mean sd)

			*Organizing
			*-----------------------------------------------------------------------------------------------------------------*
				import 		excel "$descriptives\Table3_block2.xlsx", allstring clear
				drop 		in 1
				destring, 	replace
				gen 		obs = _n
				...
				export	 excel using "$descriptives\Table B5.xls",  sheetmodify cell(A12)
				erase 	"$descriptives\Table3_block2.xlsx"
		}
		...
		*-----------------------------------------------------------------------------------------------------------------*
		{
			...
			*-----------------------------------------------------------------------------------------------------------------*
				...
				*-----------------------------------------------------------------------------------------------------------------*
				...
				using "$descriptives/Table3_block3.xlsx", stats(N mean sd) replace
				
						
				**
				**Organizing
				*-----------------------------------------------------------------------------------------------------------------*
					import 		excel "$descriptives\Table3_block3.xlsx", allstring clear
					drop 		in 1
					destring, 	replace
					gen 		obs = _n
					...
					drop order T1
					export	 excel using "$descriptives\Table B5.xls",  sheetmodify cell(A19)	
					erase 	"$descriptives\Table3_block3.xlsx"
		}
	}
	...
	{	//teachers
			...
			estimates clear
				...
				*-------------------------------------------------------------------------------------------------------------*
					...
					if `grade' == 7 replace C = "Seventh-grade" in 1
					if `grade' == 9 replace C = "Ninth-grade"   in 1
					if `grade' == 3		export	 excel using "$descriptives\Table B6.xls",  replace
					if `grade' == 5 	export	 excel using "$descriptives\Table B6.xls",  sheetmodify cell(F1)	
					if `grade' == 7 	export	 excel using "$descriptives\Table B6.xls",  sheetmodify cell(I1)	
					if `grade' == 9 	export	 excel using "$descriptives\Table B6.xls",  sheetmodify cell(L1)	
	...
	{	//students
			...
			foreach grade in 3 5 7 9 {
				...
				*-------------------------------------------------------------------------------------------------------------*
					...
					if `grade' == 7 replace C = "Seventh-grade" in 1
					if `grade' == 9 replace C = "Ninth-grade"   in 1
					if `grade' == 3		export	 excel using "$descriptives\Table B7.xls",  replace
					if `grade' == 5 	export	 excel using "$descriptives\Table B7.xls",  sheetmodify cell(F1)	
					if `grade' == 7 	export	 excel using "$descriptives\Table B7.xls",  sheetmodify cell(I1)	
					if `grade' == 9 	export	 excel using "$descriptives\Table B7.xls",  sheetmodify cell(L1)	
	...
	*---------------------------------------------------------------------------------------------------------------------*
	{
		...
		*-----------------------------------------------------------------------------------------------------------------*
		...
		sort	order
		drop 	order
		export	excel using "$descriptives\Table B10.xls",  replace
		...
		sort 	order
		drop 	order
		export	 excel using "$descriptives\Table B10.xls",  sheetmodify cell(D1)
	...
	*---------------------------------------------------------------------------------------------------------------------*
	{
		...
		*-----------------------------------------------------------------------------------------------------------------*
		...
		export	excel using "$descriptives\Table B11.xls",  replace
		...
		sort order
		drop order
		export	 excel using "$descriptives\Table B11.xls",  sheetmodify cell(D1)
		...
		sort order	
		drop order
		export	 excel using "$descriptives\Table B11.xls",  sheetmodify cell(F1)
		...
		sort order	
		drop order
		export	 excel using "$descriptives\Table B11.xls",  sheetmodify cell(H1)
	...
	*---------------------------------------------------------------------------------------------------------------------*
	//testando conjuntamente se as medias entre os grupos de T e C sao iguais. no iebaltab, testamos uma variavel de cada vez
	{
		use  "$dtfinal/Fin_Lit_pooled_data_clean.dta", clear
			...
			order B6 teste
			drop order
			export	 excel using "$descriptives\Table B12.xls",  replace
	}
